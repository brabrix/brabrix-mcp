...
export class CodeReviewPromptBuilder {
    buildPrompt(context: FullTaskContext): string {
        ...
        if (selectedItem.description) prompt += `- **Descrição:** ${selectedItem.description}\n`;
        if (selectedItem.acceptanceCriteria) prompt += `- **Critérios de Aceite:**\n${selectedItem.acceptanceCriteria}\n`;
        prompt += `\n`;
        ...
        if (userStory) {
            ...
            if (userStory.acceptanceCriteria) prompt += `- **Critérios da História:**\n${userStory.acceptanceCriteria}\n`;
            prompt += `\n`;
        }
