...
import { AgentTemplateType } from '../models/agentTemplate';
...
type OverwriteMode = 'ask' | 'overwrite' | 'backup' | 'skip';

interface SourceDescriptor {
    key: ContextSourceKey;
    title: string;
    primaryRelativePath: string;
    secondaryRelativePath?: string;
    missingHint: string;
}
...
export interface AgentTemplateGenerationOptions {
    ...
    includeQuickStarter: boolean;
    includeSkills: boolean;
    includeRules: boolean;
    overwriteMode: OverwriteMode;
}

export interface AgentTemplateGenerationResult {
    created: string[];
    updated: string[];
    skipped: string[];
    backedUp: string[];
...
export class AgentTemplateGenerator {
    ...
    previewTargetPaths(
        templateType: AgentTemplateType,
        options: Omit<AgentTemplateGenerationOptions, 'templateType' | 'overwriteMode'>
    ): string[] {
    ...
    generate(options: AgentTemplateGenerationOptions): AgentTemplateGenerationResult {
        const rootPath = this.getWorkspaceRootPath();
        ...
        const result: AgentTemplateGenerationResult = {
            created: [],
            updated: [],
            skipped: [],
            backedUp: [],
    ...
    private buildPlannedFiles(
        templateType: AgentTemplateType,
        options: Omit<AgentTemplateGenerationOptions, 'templateType' | 'overwriteMode'>,
        sourceMap: Record<ContextSourceKey, SourceDocument>
    ): PlannedOutputFile[] {
        switch (templateType) {
            case AgentTemplateType.CLAUDE_CODE:
                return this.buildClaudeFiles(options, sourceMap);
            case AgentTemplateType.CODEX:
                return this.buildCodexFiles(options, sourceMap);
            case AgentTemplateType.VSCODE_COPILOT:
                return this.buildVsCodeFiles(options, sourceMap);
            case AgentTemplateType.GEMINI_CLI:
                return this.buildGeminiFiles(options, sourceMap);
            case AgentTemplateType.GENERIC:
    ...
    private buildVsCodeFiles(
        ...
        const orderedContextKeys: ContextSourceKey[] = [
            'projectContext',
            'currentTask',
            'taskSpec',
            'prd',
    ...
    private buildGenericFiles(
        ...
        const contextMapping: Array<{ key: ContextSourceKey; filename: string }> = [
    ...
    private buildGenericTaskPrompt(
        options: Omit<AgentTemplateGenerationOptions, 'templateType' | 'overwriteMode'>,
        sourceMap: Record<ContextSourceKey, SourceDocument>
    ): string {
        const lines: string[] = [
        ...
        const orderedKeys: ContextSourceKey[] = [
            'currentTask',
            'taskSpec',
            'rules',
            'skills',
    ...
    private pushContextFiles(
        files: PlannedOutputFile[],
        folder: string,
        sourceMap: Record<ContextSourceKey, SourceDocument>,
        keys: ContextSourceKey[],
        options: Omit<AgentTemplateGenerationOptions, 'templateType' | 'overwriteMode'>
    ): void {
        const contextFileNames: Record<ContextSourceKey, string> = {
    ...
    ): Record<ContextSourceKey, SourceDocument> {
        const map = {} as Record<ContextSourceKey, SourceDocument>;
    ...
    private writePlannedFile(
        rootPath: string,
        file: PlannedOutputFile,
        overwriteMode: OverwriteMode,
        backupStamp: string,
        result: AgentTemplateGenerationResult
    ): void {
    ...
    private resolveSafeAbsolutePath(rootPath: string, relativePath: string): string {
    ...
    private buildBackupStamp(): string {
        const now = new Date();
