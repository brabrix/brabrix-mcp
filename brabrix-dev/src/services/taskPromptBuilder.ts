...
import { FullTaskContext } from './projectContextService';
...
export class TaskPromptBuilder {
    private static readonly MAX_SKILLS_CONTEXT_CHARS = 12000;
    private static readonly MAX_RULES_CONTEXT_CHARS = 12000;

    constructor(private workspaceConfig?: WorkspaceConfig) {}

    private readLocalContext(filename: string): string | undefined {
        if (!this.workspaceConfig) return undefined;
    ...
    buildTaskPrompt(context: FullTaskContext, executionProfile: string = 'plan'): string {
        ...
        if (selectedItem.priority) prompt += `- **Prioridade:** ${selectedItem.priority}\n`;
        if (selectedItem.estimatedHours) prompt += `- **Estimativa:** ${selectedItem.estimatedHours}h\n`;
        prompt += `- **Projeto:** ${project.name}\n`;
        if (project.customerName) prompt += `- **Cliente:** ${project.customerName}\n`;
        prompt += `\n`;
        ...
        if (userStory) {
            ...
            if (userStory.description) prompt += `${userStory.description}\n`;
        } else {
        ...
        prompt += this.buildOptionalContextSection(
            '## 9. Skills aplicadas ao projeto',
            localSkillsContext,
            'Nenhuma Skill sincronizada para este projeto.',
            TaskPromptBuilder.MAX_SKILLS_CONTEXT_CHARS
        );
        ...
        prompt += this.buildOptionalContextSection(
            '## 10. Rules obrigatórias do projeto',
            localRulesContext,
            'Nenhuma Rule sincronizada para este projeto.',
            TaskPromptBuilder.MAX_RULES_CONTEXT_CHARS
        );
    ...
    private buildOptionalContextSection(
        sectionTitle: string,
