...
export async function linkWorkspaceCommand(
    ...
    if (currentConfig) {
        const replace = await vscode.window.showWarningMessage(
            "Este workspace já está vinculado a um projeto Brabrix. Deseja substituir o vínculo?",
            "Substituir", "Cancelar"
        );
        if (replace !== "Substituir") return;
    }
    ...
    try {
        ...
        if (selected) {
            ...
            await syncWorkspaceCommand(client, workspaceConfig, syncStateService, refreshAll, false);
        }
    } catch (e: any) {
