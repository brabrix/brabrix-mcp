...
export async function generateMcpConfigCommand(
    ...
    try {
        ...
        vscode.window.showInformationMessage(
            `Arquivo 'gemini-settings.json' gerado com sucesso na raiz do projeto!`,
            openFile,
            copyToGemini
                ...
                try {
                    ...
                    if (fs.existsSync(geminiHomePath)) {
                        const confirm = await vscode.window.showWarningMessage(
                            `O arquivo ${geminiHomePath} já existe. Deseja substituí-lo?`,
                            "Sim", "Não"
                        );
...
export async function showMcpSetupCommand(
    workspaceConfig: WorkspaceConfig,
    syncStateService: SyncStateService
) {
    const root = workspaceConfig.getWorkspaceRoot();
...
export async function copyGeminiConfigCommand(
    workspaceConfig: WorkspaceConfig,
    syncStateService: SyncStateService,
    tokenStore: TokenStore
) {
...
export async function copyClaudeCommandCommand(workspaceConfig: WorkspaceConfig) {
    const root = workspaceConfig.getWorkspaceRoot();
    const mcpServerPath = root ? path.join(root.fsPath, 'apps', 'brabrix-mcp-server', 'dist', 'index.js') : '/CAMINHO/PARA/brabrix-mcp-server/dist/index.js';
...
export async function openMcpDocsCommand() {
