...
export async function syncWorkspaceCommand(
    ...
    if (!config) {
        if (showMessages) vscode.window.showWarningMessage("Nenhum projeto vinculado para sincronizar.");
        return;
    }
    ...
    if (!fs.existsSync(contextFolder)) fs.mkdirSync(contextFolder, { recursive: true });
    ...
    try {
        ...
        statuses.forEach(status => {
            ...
            if (items.length === 0) boardMd += `Nenhum item.\n`;
            items.forEach(item => {
        ...
        let skillsSyncResult: ProjectSkillsSyncResult | undefined;
        try {
            const skillsSyncService = new ProjectSkillsSyncService(client, workspaceConfig);
