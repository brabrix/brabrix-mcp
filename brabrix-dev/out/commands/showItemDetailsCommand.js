...
exports.showItemDetailsCommand = showItemDetailsCommand;
const vscode = require("vscode");
async function showItemDetailsCommand(client, workspaceConfig, node) {
    if (!node || !node.item) {
    ...
    const panel = vscode.window.createWebviewPanel('brabrixItemDetails', `Detalhes: ${item.title}`, vscode.ViewColumn.Two, { enableScripts: true });
    panel.webview.html = getWebviewContent(item, spec);
}
function getWebviewContent(item, spec) {
    const priorityColor = {
        'LOW': '#2ecc71',
        'MEDIUM': '#f1c40f',
        'HIGH': '#e67e22',
...
//# sourceMappingURL=showItemDetailsCommand.js.map