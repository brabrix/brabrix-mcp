...
class ProjectsTreeProvider {
    ...
    async resolveElementChildren(element) {
        const config = await this.workspaceConfig.readConfig();
        const loggedIn = await this.hasToken();
        // Filhos do Projeto
        if (element.contextValue === 'brabrixProject' && config) {
            ...
            try {
                ...
                const syncItem = new vscode.TreeItem(`Sincronização: ${state?.lastSyncAt ? new Date(state.lastSyncAt).toLocaleString() : 'Nunca'}`, vscode.TreeItemCollapsibleState.None);
